...
{{- /* Every non-empty LLM__<GROUP>[_N]__<FIELD> key renders, so numbered
fallback deployments reach the server without a template change. */}}
{{- range $name, $value := .Values.common.env }}
{{- if and (hasPrefix "LLM__" $name) $value }}
- name: {{ $name }}
  value: {{ $value | quote }}
{{- end }}
{{- end }}
